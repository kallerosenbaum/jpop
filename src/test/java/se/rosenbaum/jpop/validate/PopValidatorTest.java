...
public class PopValidatorTest extends PopTestWithWallet {
    ...
    @Test(expected = InvalidPopException.class)
    public void testValidatePopBadSignatureSecondInput() throws Exception {
        Pop pop = getPop(new int[]{1,2}, Coin.ZERO, 3);
        signPop(pop);
        List<ScriptChunk> chunks = pop.getInputs().get(1).getScriptSig().getChunks();
        chunks.get(1).data[20]++; // Mess up the signature
        validatePop(pop);
    }
    ...
    private Pop getPop(int[] fundingValue, Coin fee, int... outputValues) throws InsufficientMoneyException, IOException {
        List<Transaction> fundingTransactions = createFundingTransaction(fundingValue);
        Transaction paymentToProve = createPaymentToProve(fundingTransactions, fee, outputValues);

        return createValidUnsignedPop(fundingTransactions, paymentToProve);
    }
    ...
    private Pop createValidUnsignedPop(List<Transaction> funding, Transaction txToProve) throws IOException, InsufficientMoneyException {
        ...
        // Copy the txToProve
        Pop pop = new Pop(params, txToProve.bitcoinSerialize(), b(0, 0, 0, 0, 0, 19));
